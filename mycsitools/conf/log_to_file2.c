...
int init_local_socket(){
  ...
  strcpy(address.sun_path, "/tmp/csitools.sock");  
...
int main(int argc, char** argv)
{
	...
	local_sock = init_local_socket();
	if (local_sock == -1){
		exit_program_err(-1,"local_sock error");
	}
	...
	while (1)
	{
		...
		write(local_sock,&l2,sizeof(unsigned short));
		int r = write(local_sock,cmsg->data,l);
